...
namespace hammer {
    ...
    MBuffer::SegmentData MBuffer::SegmentData::slice(size_t start, size_t length) {
        ...
        HAMMER_ASSERT(m_length >= start);
        HAMMER_ASSERT(m_length >= length + start);
    ...
    m_writeIndex(0),
    m_data(length) {
        HAMMER_ASSERT(m_writeIndex <= m_data.m_length);
    }
    ...
    void MBuffer::Segment::produce(size_t length) {
        HAMMER_ASSERT(length <= writeAvailable());
        m_writeIndex += length;
        HAMMER_ASSERT(m_writeIndex <= m_data.m_length);
    }

    void MBuffer::Segment::consume(size_t length) {
        HAMMER_ASSERT(length <= readAvailable());
        m_writeIndex -= length;
    ...
    void MBuffer::Segment::truncate(size_t length) {
        HAMMER_ASSERT(length <= readAvailable());
        HAMMER_ASSERT(m_writeIndex == readAvailable());
        m_writeIndex = length;
    ...
    void MBuffer::compact() {
        ...
        HAMMER_ASSERT(writeAvailable() == 0);
    }
    ...
    void MBuffer::clear(bool clearWriteAvailAsWell) {
        ...
        invariant();
        HAMMER_ASSERT(m_readAvailable == 0);
    }
    ...
    void MBuffer::truncate(size_t length) {
        ...
        while (it != m_segments.end() && it->readAvailable() > 0) {
            HAMMER_ASSERT(it->writeAvailable() == 0);
    ...
    iovec MBuffer::writeBuffer(size_t length, bool reallocate) {
        ...
        if (writeAvailable() == 0) {
            reserve(length);
            HAMMER_ASSERT(m_writeIt != m_segments.end());
            HAMMER_ASSERT(m_writeIt->writeAvailable() >= length);
    ...
    void MBuffer::copyIn(const MBuffer& buffer, size_t length, size_t pos) {
        if (pos > buffer.readAvailable())  {
            HAMMER_ASSERT("positioin out of range");
            return;
        }
        if (length == (size_t)~0) {
            length = buffer.readAvailable() - pos;
        }
        HAMMER_ASSERT(buffer.readAvailable() >= length + pos);
        invariant();
        if (length == 0)
            return;
        if (m_writeIt != m_segments.end() && m_writeIt->readAvailable() != 0) {
        ...
        HAMMER_ASSERT(it != buffer.m_segments.end());
        for (; it != buffer.m_segments.end(); ++it) {
            ...
            if (m_readAvailable != 0 && it == buffer.m_segments.begin()) {
                ...
                if ((char*)previousIt->readBuffer().getStart() +
                    ...
                    HAMMER_ASSERT(previousIt->writeAvailable() == 0);
        ...
        HAMMER_ASSERT(length == 0);
        HAMMER_ASSERT(readAvailable() >= length);
    }
    ...
    void MBuffer::copyOut(void* buffer, size_t length, size_t pos) const {
        ...
        HAMMER_ASSERT(length + pos <= readAvailable());
        ...
        HAMMER_ASSERT(it != m_segments.end());
    ...
    std::string MBuffer::getDelimited(char delimiter, bool eofIsDelimiter, 
        ...
        HAMMER_ASSERT(offset >= -1);
        if (offset == -1 && !eofIsDelimiter) {
            HAMMER_ASEERT("getDelimited failed");
        }
...
#undef XX
    ...
    int MBuffer::opCmp(const MBuffer& rhs) const {
        ...
        while (leftIt != m_segments.end() && rightIt != rhs.m_segments.end()) {
            HAMMER_ASSERT(leftOffset <= leftIt->readAvailable());
            HAMMER_ASSERT(rightOffset <= rightIt->readAvailable());
    ...
    void MBuffer::invariant() const {
        ...
        for (it = m_segments.begin(); it != m_segments.end(); it++) {
            ...
            HAMMER_ASSERT(!seenWrite || (seenWrite && segment.readAvailable() == 0)); // 确保前面的块是"实心"的
            read += segment.readAvailable();
            write += segment.writeAvailable();
            if (!seenWrite && segment.writeAvailable() != 0) { // 这个块没有满即有剩余空间
                seenWrite = true;
                HAMMER_ASSERT(m_writeIt == it);                  // 不满得这个块是当前块
            }
            ...
            if (nextIt != m_segments.end()) {
                ...
                if (segment.writeAvailable() == 0 &&  // 当前块是满的 且下一块有数据
                    ...
                    HAMMER_ASSERT((const unsigned char*)segment.readBuffer().start() +
                    ...
                    next.readAvailable() == 0) {
                    HAMMER_ASSERT((const unsigned char*)segment.writeBuffer().start() +
                        segment.writeAvailable() != next.writeBuffer().start() ||
                        segement.m_data.m_array.get() != next.m_data.m_array.get());
                }
            }
        }
        HAMMER_ASSERT(read == m_readAvailable);
        HAMMER_ASSERT(write == m_writeAvailable);
        HAMMER_ASSERT(write != 0 || (write == 0 && m_writeIt == m_segments.end()));
    #endif
    }
}
